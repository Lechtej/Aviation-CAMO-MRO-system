...
## Cel etapu 2 — Aircraft utilization ledger (FH/FC) + counters (public)

- Dodać audytowalne śledzenie nalotu i cykli (FH/FC) jako **append-only ledger**.
- Dodać snapshot (`aircraft_counters`) do szybkiego planowania i UI.
- Dodać daty w `public.aircraft` do wyliczania wieku (pochodne).

### Pliki dodane w tej paczce
- `db/migrations/public/0004_public_aircraft_utilization.sql`

### Inwarianty (must hold)
- `delta_fh >= 0`, `delta_fc >= 0` w ledgerze
- unikalność wpisu:
  - `(aircraft_id, source, source_ref)` gdy `source_ref` podany
  - `(aircraft_id, op_date, source)` gdy `source_ref` pusty
- `total_fh/total_fc` w counters są nieujemne
...
## 2026-01-14 — Idempotency & Reservation Consistency

- **Invariant**: `stock_items.qty_reserved` equals SUM(`qty_reserved - qty_consumed`) over OPEN reservations.
- **Control**: recalculation executed before reservation availability checks.
- **Idempotency**: enforced per-tenant to maintain isolation guarantees.
