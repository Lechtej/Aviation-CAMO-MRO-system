...
## v0.2.46 (2026-01-14) — AIRCRAFT: utilization ledger + counters (FH/FC)

- DB: added **public** utilization tracking:
  - `public.aircraft_utilization_ledger` (append-only; unique by source_ref or by date+source)
  - `public.aircraft_counters` (snapshot totals for fast reads/planning)
  - extended `public.aircraft` with optional `manufacture_date` and `entry_into_service_date`
- API: new endpoints:
  - `POST /v1/aircraft/{aircraft_id}/utilization`
  - `GET  /v1/aircraft/{aircraft_id}/utilization`
  - `GET  /v1/aircraft/{aircraft_id}/counters`
- API: aircraft responses now include optional fields: `total_fh`, `total_fc`, `manufacture_date`, `entry_into_service_date`.
...
- UI: deterministic token lifecycle handling (no silent refresh):
  - preflight check: block protected `/v1/*` calls when token is missing/expired
  - global 401 handler: clear stored auth and show controlled re-login UX
