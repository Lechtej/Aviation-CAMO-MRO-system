...
REM - Or run: start_and_test_v0.2.11.bat /diag
REM ============================================================
...
REM Aviation CAMO/MRO - One-click start + smoke test (v0.2.11)
...
set "LOG_FILE=%LOG_DIR%\start_and_test_v0.2.11_%TS%.log"
...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  "$t = ($env:BEARER_TOKEN).ToString().Trim(); " ^
  "if ([string]::IsNullOrWhiteSpace($t)) { Write-Host 'ERROR: Empty token'; exit 3 } " ^
  "$hdr = @{ Authorization = ('Bearer ' + $t) };" ^
  "try { Invoke-RestMethod -ErrorAction Stop -Headers $hdr -Uri '%API_BASE%/v1/tenants' | ConvertTo-Json -Depth 10 } catch { " ^
...
for /f "delims=" %%B in ('powershell -NoProfile -Command ^
  ...
  "$headers=@{Authorization=('Bearer ' + ($env:BEARER_TOKEN).ToString().Trim()); 'X-Tenant-Id'='00000000-0000-0000-0000-000000000000'}; " ^
